...
public class Year2025_Day09_color(string inputPath) : Solvable($"{inputPath}/input") {
    ...
    public override (object, object) Solve() {
        ...
        tiles.ForEach(t => (t.rx, t.ry) = (xValues.IndexOf(t.x), yValues.IndexOf(t.y)));

        var (maxX, maxY) = (lines.Max(l => Math.Max(l.Item1.rx, l.Item2.rx)), lines.Max(l => Math.Max(l.Item1.ry, l.Item2.ry)));
        int[,] grid = new int[maxX + 1, maxY + 1];
        for (int y = 0; y <= maxY; y++) {
            for (int x = 0; x <= maxX; x++) {
                grid[x, y] = tiles.Any(t => t.rx == x && t.ry == y) ? 1 : lines.Any(l => l.Item1.rx <= x && l.Item2.rx >= x && l.Item1.ry <= y && l.Item2.ry >= y) ? 2 : 0;
            }
        }
    ...
    private void DumpGrid(int[,] grid) {
        ...
        for (long y = 0; y < maxY; y++) {
            for (long x = 0; x < maxX; x++) {
                ...
                (int r, int g, int b, char c) = v switch {
                    1 => (240, 0, 0, '#'),
                    2 => (0, 240, 0, 'X'),
                    3 => (0, 240, 0, 'X'),
                    100 => (240, 0, 240, '#'),
